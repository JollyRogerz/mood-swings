// Mood Swings-inspired flat V2 for one 45-card sleeved deck.
// The long, in-plane side clip on the lid is a positive travel lock. Pull the
// thumb pad away from the right side of the body to release it before sliding.
// Separate body and lid print on their broad, flat faces with no supports.
...
lid_t = 2.25;
H = floor + stack + 5;
latch_notch_start = 33.0;
latch_notch_end = 37.7;
latch_notch_depth = 0.75;
latch_engagement = 0.55;
latch_beam_width = 1.4;
$fn = 32;
...
module body() {
  ...
  difference() {
    ...
    // Recess in the outside of the right rail. Its front face is the hard
    // stop that catches the lid's hook when the lid tries to slide out.
    // The short roof above the recess bridges just 0.75 mm when printed.
    translate([W/2-latch_notch_depth,latch_notch_start,H-5])
      cube([latch_notch_depth+0.2,
            latch_notch_end-latch_notch_start,
            3.05]);
  }
  side_art();
  }
...
module in_plane_latch() {
  beam_x = W/2+0.4;
  // A long, flat-printed beam gives silk PLA a small, repeatable deflection.
  // At rest the hook projects 0.55 mm into the body side; its shallow
  // approach ramp cams the beam out during insertion.
  translate([beam_x,36.8,0])
    cube([latch_beam_width,16.7,lid_t]);
  linear_extrude(height=lid_t)
    polygon([[beam_x,32.7],
             [W/2-latch_engagement,35.2],
             [W/2-latch_engagement,37.4],
             [beam_x+1.0,37.4]]);
  // Round the loaded root so it has no sharp re-entrant corner.
  translate([beam_x+latch_beam_width/2,51.7,0])
    cylinder(h=lid_t,r=1.2,$fn=32);
  // Exposed pad: hook it outward about 1 mm, then pull the lid's end stop.
  translate([beam_x+0.7,33.8,0])
    cube([2.25,4.5,lid_t]);
  for (y=[34.2:1.15:37.65])
    translate([beam_x+latch_beam_width+0.3,y,lid_t-0.02])
      cube([0.9,0.42,0.38]);
}
module lid() {
  ...
  union() {
    ...
    translate([-W/2,card_h/2+wall+0.1,0])
      cube([W,3,5]);
    // Extend the stop to anchor the cantilever outside the tray wall.
    translate([W/2-0.2,card_h/2+wall+0.1,0])
      cube([2.2,3,5]);
    in_plane_latch();
    lid_art();
...
module fit_gauge() {
  // This coupon retains the actual mouth, stop, recess, spring beam, ramp,
  // and release pad. It tests a real locked closure rather than just rails.
  translate([-50,-40,-(H-11)])
    intersection() {
      body();
      translate([-W/2-4,28.5,H-11]) cube([W+9,26,11.1]);
    }
  // Mating 26 mm lid section includes the full-length external side clip.
  translate([50,-40,0])
    intersection() {
      lid();
      translate([-W/2-4,28.5,-0.1]) cube([W+9,26,6]);
    }
