...
| `latch_gauge_body.stl`, `latch_gauge_lid.stl` | Compact working sample of the actual rail, both snap pockets, printable spring roots and pinch ends. **Print and cycle this first.** |
...
| `latch_only_plate.3mf` | Both latch coupon parts without the card ring, for a shorter retest after checking card fit. |
...
1. Print `latch_gauges_plate.3mf` first using your **K1C 2025, 0.4 mm nozzle, actual silk PLA filament profile** and a clean build plate. If the card-fit ring has already been printed and checked, print `latch_only_plate.3mf` instead. The detail process has 0.24 mm layers, three walls, five top/bottom layers, 12% gyroid, 5 mm brims and no supports. The faster 0.28 mm process also keeps three walls and uses four top/bottom layers. A local OrcaSlicer slice of the revised faster full plate estimated **3 h 24 min 17 s and 87.03 g**; the original three-part coupon plate estimated **38 min 55 s and 13.89 g**. The coupon estimate predates its correction and must be rechecked in the slicer. Follow the temperature range on the spool. Each coupon STL can also be printed alone.
...
**Closure revision:** An earlier latch coupon was cropped at X=7.5 mm, only 0.05 mm behind the rounded ends of the spring slits. That bridge was too thin to print with a 0.4 mm nozzle, so its latch test could not establish whether the full lid would hold. The revised body and lid coupons both start at X=0 mm, leaving 7.55 mm of solid lid material behind each slit root. The full-size lid already had 42.2 mm of uncut material behind each root and is unchanged. The full-size body's latch pockets now extend 0.05 mm beyond the channel floor and roof, so the nubs are not precompressed when the lid sits at the bottom of its vertical play. Regenerate G-code from `case_plate.3mf` if an older slice was prepared. Digital checks do not replace a physical fit test for a specific spool.
...
OpenSCAD and Blender verification found **one watertight component and zero nonmanifold edges** in every STL. A measured **66 × 92 × 31 mm** virtual deck and the closed lid have **zero intersecting volume** with the body. The small overlap while the lid slides is deliberately confined to the two snap nubs; the coupon reproduces the same **2.658 mm³** interference at a +2 mm slide offset. A sweep across the full slide path at the channel floor, 0.25 mm above it and the roof-side limit found no closed-position collision; peak intended nub interference was **2.690 mm³** at a +4 mm slide offset, and the path was clear from +8 mm onward. The minimum deck-to-lid vertical gap was **4.00 mm**. These checks validate CAD geometry; a physical fit and loaded-case test are still needed to establish real tolerances and retention. Printing the full case directly is possible, but the actual latch fit will only be known after it cools.
