"""Build geometry-only 3MF plates for the case and latch gauges.
...
def stl_mesh(path: Path):
    ...
    for triangle in range(triangles):
        ...
        for corner in range(3):
            raw = struct.unpack_from("<fff", data, offset + corner * 12)
            point = (raw[0], raw[1], 0.0 if abs(raw[2]) < 0.001 else raw[2])
...
for object_id, name, stl_name, x, y in (
    (1, "Closure gauge - body", "latch_gauge_body.stl", 60, 110),
    (2, "Closure gauge - lid", "latch_gauge_lid.stl", 150, 110),
    (3, "Card fit ring", "card_gauge.stl", 110, 40),
):
...
target = ROOT / "latch_gauges_plate.3mf"
